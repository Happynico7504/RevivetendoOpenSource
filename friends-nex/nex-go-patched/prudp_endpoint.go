...
func (pep *PRUDPEndPoint) handleConnect(packet PRUDPPacketInterface) {
	...
	fmt.Printf("[PRUDP] handleConnect: addr=%s state=%d isSecure=%v payloadLen=%d sessID=%02x connSig=%x\n",
		connection.Socket.Address.String(), connection.ConnectionState, pep.IsSecureEndPoint, len(packet.Payload()),
		packet.SessionID(), packet.GetConnectionSignature())
	...
	// Log requestData bytes regardless of IsSecureEndPoint so we can correlate with auth sessionKey
	if len(packet.Payload()) == 36 {
		raw := packet.Payload()
		// ticketData length (4 bytes) at [0:4], then requestData length (4) + data (28) at [4:36]
		if raw[0] == 0 && raw[1] == 0 && raw[2] == 0 && raw[3] == 0 &&
			raw[4] == 28 && raw[5] == 0 && raw[6] == 0 && raw[7] == 0 {
			fmt.Printf("[PRUDP-CONNECT] sessID=%02x connSig=%x requestData=%x\n",
				packet.SessionID(), packet.GetConnectionSignature(), raw[8:36])
		}
	}
	...
	if pep.IsSecureEndPoint {
		...
		clientConnSig := packet.GetConnectionSignature()
		fmt.Printf("[FRIENDS-KERBEROS] client connSig: %x\n", clientConnSig)
		sessionKey, pid, checkValue, err := pep.ReadKerberosTicket(decompressedPayload, clientConnSig)
		if err != nil {
			logger.Error(err.Error())
			return
		}
...
func (pep *PRUDPEndPoint) handleDisconnect(packet PRUDPPacketInterface) {
	...
	conn := packet.Sender().(*PRUDPConnection)
	fmt.Printf("[PRUDP] handleDisconnect: addr=%s state=%d pid=%d isSecure=%v\n",
		conn.Socket.Address.String(), conn.ConnectionState, uint64(conn.PID()), pep.IsSecureEndPoint)
...
func (pep *PRUDPEndPoint) ReadKerberosTicket(payload []byte, clientConnSig []byte) ([]byte, types.PID, uint32, error) {
	fmt.Printf("[FRIENDS-KERBEROS] ReadKerberosTicket payload (%d bytes): %x\n", len(payload), payload)
	...
	if err := ticketData.ExtractFrom(stream); err != nil {
		fmt.Printf("[FRIENDS-KERBEROS] ticketData ExtractFrom error: %v\n", err)
		return nil, 0, 0, err
	}
	fmt.Printf("[FRIENDS-KERBEROS] ticketData len=%d\n", len(ticketData))

	requestData := types.NewBuffer(nil)
	if err := requestData.ExtractFrom(stream); err != nil {
		fmt.Printf("[FRIENDS-KERBEROS] requestData ExtractFrom error: %v\n", err)
		return nil, 0, 0, err
	}
	fmt.Printf("[FRIENDS-KERBEROS] requestData len=%d\n", len(requestData))

	// When the Wii U Friends service can't decrypt the outer Kerberos ticket (wrong
	// password), it sends a 0-length ticketData but still sends requestData encrypted
	// with its (garbage) session key.  We can't decrypt requestData properly, but we
	// can still let the CONNECT succeed: use a zero session key, attempt decryption
	// (ignoring HMAC), and extract a best-effort checkValue so the CONNECT ACK contains
	// a plausible response.  PID=0 here; the Register handler fills it from StationURL.
	if len(ticketData) == 0 {
		fmt.Printf("[FRIENDS-KERBEROS] 0-length ticket — requestData[0:12]=%x HMAC=%x\n",
			requestData[:12], requestData[12:])

		// Try candidate keys to decrypt requestData = RC4(key, {PID(4)+CID(4)+checkValue(4)}) + HMAC(16).
		// Log each attempt so we can identify the right key from journals.
		type candidate struct {
			key   []byte
			label string
		}
		zeroKey := make([]byte, pep.Server.SessionKeyLength)
		candidates := []candidate{
			{zeroKey, "zero16"},
			{[]byte("CD&ML"), "CD&ML"},
			{clientConnSig, "clientConnSig"},
		}
		for _, c := range candidates {
			if len(c.key) == 0 {
				continue
			}
			cip, err2 := rc4.NewCipher(c.key)
			if err2 != nil {
				continue
			}
			plain := make([]byte, len(requestData))
			cip.XORKeyStream(plain, requestData[:12])
			// Verify HMAC
			mac := hmac.New(md5.New, c.key)
			mac.Write(requestData[:12])
			wantHMAC := mac.Sum(nil)
			hmacOK := hmac.Equal(wantHMAC, requestData[12:])
			pidFromPlain := binary.LittleEndian.Uint32(plain[0:4])
			checkV := binary.LittleEndian.Uint32(plain[8:12])
			fmt.Printf("[FRIENDS-KERBEROS] key=%s(%x): PID=%d chk=%d HMAC=%v\n",
				c.label, c.key, pidFromPlain, checkV, hmacOK)
		}
		// Fall through using zero session key; Register RMC will provide the real PID.
		return zeroKey, 0, 0, nil
	}
